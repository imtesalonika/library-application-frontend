...
export function TugasakhirPage() {
    ...
    const getTugasAkhirData = async () => {
        try {
          const response = await axios.get(`${apiUrl}/api/tugasakhir`)
    ...
    const handleRemoveTugasAkhir = async (tugasakhirId: number) => {
        try {
          const response = await axios.delete(`${apiUrl}/api/tugasakhir/${tugasakhirId}`)
    ...
    return (
        <div className={'p-4 bg-white'}>
            ...
            >
                ...
                <tbody>
                    {tugasakhirData?.map((row: any, index: any) => (
                        <tr key={index}>
                            <td>{index + 1}</td>
                            <td>{row.judul}</td>
                            <td>{row.prodi}</td>
                            <td className={'d-flex'} style={{ gap: '5px' }}>
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                    navigation(`/tugasakhir/edit/${row.id}`)
                                    }}
                                >
                                    <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                    handleRemoveTugasAkhir(row.id).then()
                                    }}
                                >
                                    <svg
                                    xmlns="http://www.w3.org/2000/svg"
